"""Daily market summary and a small auditable candlestick confirmation set."""

def daily_summary(d1, price):
    previous, today = d1[-2], d1[-1]
    prev_close=float(previous['close']); open_=float(today['open']); high=float(today['high']); low=float(today['low'])
...
def candle_signals(bars):
    prev,cur=bars[-2],bars[-1]; o,h,l,c=map(float,(cur['open'],cur['high'],cur['low'],cur['close'])); po,pc=map(float,(prev['open'],prev['close']))
    body=max(abs(c-o),1e-9); upper=h-max(o,c); lower=min(o,c)-l; signals=[]
    ...
    prior=bars[-22:-2]
    resistance=max(float(x['high']) for x in prior); support=min(float(x['low']) for x in prior)
...
def multi_candle_signals(candles):
    return {tf:candle_signals(candles[tf]) for tf in ('M5','M15','M30','H1')}
