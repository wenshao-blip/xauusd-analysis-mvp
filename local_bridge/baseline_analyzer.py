...
def analyze(market,news,valid_until,run_type='scheduled'):
    ...
    # A weak directional edge must not suppress a valid range candidate.  The
    # candidate layer evaluates its own regime, confirmation and R:R gates.
    strategy=select_strategy(market,news,direction,allow_trade=not stale)
    market['indicators']['_strategy']=strategy
    decision='trade' if strategy['name']!='flat' else 'flat'
    ...
    # Version bump is mandatory because candidate ranking and report selection
    # changed.  Old session-3x-v1 observations remain frozen and are never mixed.
    model_version='candidate-explain-v1' if run_type=='scheduled_session' else 'candidate-explain-v1-manual'
    return {'valid_until':valid_until,'decision':decision,'direction':direction,'p_up':round(p_up,4),'p_range':round(p_range,4),'p_down':round(p_down,4),'target_low':round(low,2),'target_high':round(high,2),'execution':execution,'abandon':abandon,'strategy':strategy,'run_type':run_type,'model_version':model_version}
