...
def main():
    ap=argparse.ArgumentParser();ap.add_argument('--symbol',default='XAUUSD');ap.add_argument('--hours',type=int,default=6);ap.add_argument('--manual',action='store_true');ap.add_argument('--no-push',action='store_true');a=ap.parse_args()
    now=datetime.now(timezone.utc); valid=now+timedelta(hours=a.hours); raw=mt5_snapshot(a.symbol); raw['indicators']=multi_timeframe(raw['candles']); news=news_collect(); db=connect(DB)
    # 上个区间用当前时刻前最后一段M5行情结算；正式版可替换为Tick路径。
    m5=raw['candles']['M5']; settle_due(db,iso(now),float(m5[-1]['close']),max(float(x['high']) for x in m5[-max(2,a.hours*12):]),min(float(x['low']) for x in m5[-max(2,a.hours*12):]))
